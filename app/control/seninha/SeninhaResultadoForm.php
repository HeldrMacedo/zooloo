...
class SeninhaResultadoForm extends TPage
{
    const FILTRO_BANCA = IntJogo::FILTRO_BANCA_SENINA;
    const DEZENAS      = 6;
    const MASK         = '00,00,00,00,00,00';
    ...
    public function __construct()
    {
        ...
        $this->form = new BootstrapFormBuilder('form_seninha_resultado');
        $this->form->setFormTitle('Seninha — Lançar Resultado');
        ...
        $numeros_sorteados->placeholder = '00,00,00,00,00,00';
        ...
        $label = new TLabel('Números Sorteados (6 dezenas):');
        $label->setSize('100%');
    ...
    /**
     * Valida se a extração selecionada pertence ao filtro_banca = 3 (Seninha).
     */
    public static function assertExtracaoSeninha($extracao_id): void
    {
        $extracao = new Extracao($extracao_id);
        if ((int) $extracao->filtro_banca !== self::FILTRO_BANCA) {
            throw new Exception('Extração informada não pertence à Seninha.');
        }
    }

    /**
     * Valida o formato dos números sorteados ("00,00,00,00,00,00").
    ...
    public function onSearch($param)
    {
        ...
        try {
            ...
            self::assertExtracaoSeninha($data->extracao_id);
    ...
    public function onEdit($param)
    {
        ...
        try {
            ...
            self::assertExtracaoSeninha($sorteio->extracao_id);
            TTransaction::close();
