...
class QuininhaModalidadeList extends TStandardList
{
    ...
    public function __construct()
    {
        ...
        $this->form = new BootstrapFormBuilder('form_search_quininha_modalidade');
        $this->form->setFormTitle('Quininha — Configuração de Modalidade');
        ...
        $col_quina        = new TDataGridColumn('multiplicador_colocacao_01', 'Prêmio Quina (5 ac.)', 'right');
        $col_quadra       = new TDataGridColumn('multiplicador_colocacao_02', 'Prêmio Quadra (4 ac.)', 'right');
        $col_terno        = new TDataGridColumn('multiplicador_colocacao_03', 'Prêmio Terno (3 ac.)', 'right');
        ...
        $col_terno->setTransformer($format_brl);
        ...
        $this->datagrid->addColumn($col_terno);
        $this->datagrid->addColumn($col_ativo);

        $action_edit = new TDataGridAction(['QuininhaModalidadeForm', 'onEdit'], ['key' => '{modalidade_id}', 'register_state' => 'false']);
        ...
        $panel = new TPanelGroup('Quininha — Configuração de Modalidade');
    ...
    public function onReload($param = null)
    {
        try
        {
            ...
            $filto_banca = IntJogo::FILTRO_BANCA_QUINHA;
            $modalidades = Modalidade::where("jogo_id", 'in', "(SELECT jogo_id FROM int_jogo WHERE filtro_banca = :[{$filto_banca}]:)")->load();
