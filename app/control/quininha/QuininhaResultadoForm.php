...
class QuininhaResultadoForm extends TPage
{
    const FILTRO_BANCA = 2;
    const DEZENAS      = 5;
    const MASK         = '00,00,00,00,00';
    ...
    public function __construct()
    {
        ...
        $this->form = new BootstrapFormBuilder('form_quininha_resultado');
        $this->form->setFormTitle('Quininha — Lançar Resultado');
        ...
        //$hora_sorteio->style = 'display:none';
        ...
        $numeros_sorteados->placeholder = '00,00,00,00,00';
        ...
        $this->form->addFields([new TLabel('Data Sorteio:')], [$data_display], [New TLabel('Hora Sorteio:')], [$hora_sorteio]);
        $this->form->addContent(['<hr>']);
        $label = new TLabel('Números Sorteados (5 dezenas):');
        $label->setSize('100%');
    ...
    /**
     * Valida se a extração selecionada pertence ao filtro_banca = 2 (Quininha).
     */
    public static function assertExtracaoQuininha($extracao_id): void
    {
        $extracao = new Extracao($extracao_id);
        if ((int) $extracao->filtro_banca !== self::FILTRO_BANCA) {
            throw new Exception('Extração informada não pertence à Quininha.');
        }
    }

    /**
     * Valida o formato dos números sorteados ("00,00,00,00,00").
     * Lança Exception caso o valor seja inválido.
     */
    ...
    public function onSearch($param)
    {
        ...
        try {
            ...
            self::assertExtracaoQuininha($data->extracao_id);
    ...
    public function onEdit($param)
    {
        ...
        try {
            ...
            self::assertExtracaoQuininha($sorteio->extracao_id);
            TTransaction::close();
