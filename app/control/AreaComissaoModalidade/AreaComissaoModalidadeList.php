...
class AreaComissaoModalidadeList extends TStandardList
{
    protected $form;
    protected $datagrid;
    ...
    public function __construct()
    {
        ...
        parent::setActiveRecord('AreaComissaoModalidade');
        parent::setDefaultOrder('area_comissao_modalidade_id');
        parent::addFilterField('area_comissao_modalidade_id', '=', 'area_comissao_modalidade_id');
        parent::addFilterField('area_id', '=', 'area_id');
        parent::addFilterField('modalidade_id', '=', 'modalidade_id');
        parent::addFilterField('comissao', '>=', 'comissao_min');
        parent::addFilterField('comissao', '<=', 'comissao_max');
        ...
        // Formulário de busca
        $this->form = new BootstrapFormBuilder('form_search_area_comissao_modalidade_list');
        ...
        $comissao_min = new TEntry('comissao_min');
        $comissao_max = new TEntry('comissao_max');

        $this->form->addFields([new TLabel('Id')], [$id]);
        $this->form->addFields([new TLabel('Área')], [$area_id]);
        $this->form->addFields([new TLabel('Modalidade')], [$modalidade_id]);
        $this->form->addFields([new TLabel('Comissão Mín. (%)')], [$comissao_min], [new TLabel('Comissão Máx. (%)')], [$comissao_max]);

        $id->setSize('30%');
        $area_id->setSize('100%');
        $modalidade_id->setSize('100%');
        $comissao_min->setSize('100%');
        $comissao_max->setSize('100%');
        ...
        $column_id = new TDataGridColumn('area_comissao_modalidade_id', 'Id', 'center', 50);
        ...
        // Transformador para formatação de porcentagem
        ...
        $order_id->setParameter('order', 'area_comissao_modalidade_id');
        ...
        // Ações do datagrid
        $action_edit = new TDataGridAction(['AreaComissaoModalidadeForm', 'onEdit'], ['register_state' => 'false']);
        $action_edit->setButtonClass('btn btn-default');
        $action_edit->setLabel(_t('Edit'));
        $action_edit->setImage('far:edit blue');
        $action_edit->setField('area_comissao_modalidade_id');
        ...
        $action_del->setField('area_comissao_modalidade_id');
        ...
        // Botões do header
        $panel->addHeaderActionLink('', new TAction(['AreaComissaoModalidadeForm', 'onEdit'], ['register_state' => 'false']), 'fa:plus');
    ...
    public function Delete($param)
    {
        try
        {
            ...
            $object = new AreaComissaoModalidade($key, FALSE);
            $object->delete();
            TTransaction::close();
