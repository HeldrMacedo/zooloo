...
class SeninhaResultadoForm extends TPage
{
    ...
    public function __construct()
    {
        ...
        try {
            ...
            $rows = $conn->query(
                ...
                "WHERE data_sorteio = CURRENT_DATE AND situacao = 'A' ORDER BY descricao"
            );
            $opcoes = [];
            foreach ($rows as $row) {
        ...
        $this->form->addFields([$sorteio_id], [$hora_sorteio]);
        ...
        $this->form->addContent(['<hr>']);
        $this->form->addFields([new TLabel('Números Sorteados (6 dezenas):')], [$numeros_sorteados]);
    ...
    public function onEdit($param)
    {
        ...
        try {
            ...
            $obj->extracao_id      = $sorteio->extracao_id;
    ...
    public function onSave($param)
    {
        try {
            ...
            if (!empty($hora) && date('H:i:s') < $hora) {
                throw new Exception("O sorteio só pode ser lançado após às {$hora}");
            }

            $numeros = trim($data->numeros_sorteados ?? '');
            ...
            $sorteio->numeros_sorteados = $numeros;
            $sorteio->store();
            TTransaction::close();
